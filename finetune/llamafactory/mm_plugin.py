            ...
            if (
                ...
                while VIDEO_PLACEHOLDER in content:
                    ...
                    video_t_index = (
                        ...
                        .expand(
                            -1,
                            video_grid_thw[num_video_tokens][1] // processor.video_processor.merge_size,
                            video_grid_thw[num_video_tokens][2] // processor.video_processor.merge_size,
                        )
                        .flatten()
                        * mm_inputs["video_second_per_grid"][num_video_tokens]
                        * 13  # FIXME hardcode of position_id_per_seconds=13
                    ).long()
                    # t_ntoken_per_chunk = 50  # FIXME hardcode: [25 * 2]
                    # video_chunk_indices = processor.get_chunked_index(video_t_index, t_ntoken_per_chunk)
                    # audio_chunk_indices = processor.get_chunked_index(audio_t_index, t_ntoken_per_chunk)
                    ...
                    while video_data_index < len(video_t_index) and audio_data_index < len(audio_t_index):
                        if video_t_index[video_data_index] <= audio_t_index[audio_data_index]:
                            placeholder_string += self.video_token
                            video_data_index += 1
                        else:
                            placeholder_string += self.audio_token
                            audio_data_index += 1
                    if video_data_index < len(video_t_index):
                        placeholder_string += self.video_token * (
                            len(video_t_index) - video_data_index
                        )
                    if audio_data_index < len(audio_t_index):
                        placeholder_string += self.audio_token * (
                            len(audio_t_index) - audio_data_index
                        )
